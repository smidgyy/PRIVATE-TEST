...
console.log(">>> [BOOT] Server process started at:", (/* @__PURE__ */ new Date()).toISOString());
...
var MockFirestore = class {
  ...
  collection(name) {
    if (!this.storage.has(name)) {
      this.storage.set(name, /* @__PURE__ */ new Map());
    }
...
<body>
  ...
  app.get("/api/getNode02", async (req, res) => {
    try {
      const userId = req.session?.userId;
      if (!userId) return res.status(401).json({ status: "error", message: "Unauthorized" });
      const db = await getDb();
...
Stage 4 unlocked. Messenger updated.`,
    ...
    try {
      ...
      const contentMap = {
        ...
        "vale-photo-data": {
          ...
          content: "Coordinates: 42.3601\xB0 N, 71.0589\xB0 W | Fragment: 0x445A"
        },
        "forum": {
          access: userData.stage4_unlocked || false,
  ...
  app.get(protectedRoutes, async (req, res, next) => {
    try {
      if (req.path.startsWith("/api") || req.path.includes(".js") || req.path.includes(".css") || req.path.includes(".png") || req.path.includes(".jpg") || req.path.includes(".svg") || req.path.includes(".woff2") || req.path.includes(".mp3")) {
        return next();
      }
